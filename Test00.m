...
input_my_processing = 'data_20190626-191536.rgb24';

s_width = 108;
s_height = 192;
rgb_frame_size = s_width * s_height * 3;

f_ffmpeg_raw_hndlr = dir(fullfile(input_path, input_ffmpeg_raw));
...
f_my_processing_hndlr = dir(fullfile(input_path, input_my_processing));
...
for i=1:1:min([1000, n_frames_ffmpeg_raw, n_frames_my_processing])
    subplot(1,2,1), imshow(frames_ffmpeg_raw{i}), title(sprintf('Frame %d' ,i));
    subplot(1,2,2), imshow(frames_my_processing{i}), title(sprintf('Frame %d' ,i));
    pause(0.05);
end
